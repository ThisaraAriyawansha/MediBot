...
<body>
    ...
    <div id="contact-container">
        <section id="contact-info">
            ...
            <h2>Enter Medicine Values</h2>
            <label for="counter1" class="counter-label">Paracetamol :</label>
            <input type="number" value="0" id="Paracetamol" name="Paracetamol" class="counter-input"><br>
            <label for="counter2" class="counter-label">Aspirin :</label>
            <input type="number" value="0" id="Aspirin" name="Aspirin" class="counter-input"><br>
            <label for="counter3" class="counter-label">Metformin :</label>
            <input type="number" value="0" id="Metformin" name="Metformin" class="counter-input"><br>
            <label for="counter4" class="counter-label">Prednisone :</label>
            <input type="number" value="0" id="Prednisone" name="Prednisone" class="counter-input"><br>
            <label for="counter5" class="counter-label">Acetaminophen Syrup:</label>
            <select id="Syrup" name="Syrup" class="counter-input" placeholder="Choose..">
        ...
        &nbsp&nbsp&nbsp
<button type="button" onclick="submitForm()" id="submitButton" style="background-color: #04274c; color: white; border: none; border-radius: 20px; padding: 12px 20px; font-size: 16px;">Submit</button>
    ...
    function submitForm() {
    const maxRfidValue = document.getElementById("maxRfidValue").value; // Get RFID value
    const Paracetamol = document.getElementById("Paracetamol").value; // Get Paracetamol value
    const Aspirin = document.getElementById("Aspirin").value; // Get Aspirin value
    const Metformin = document.getElementById("Metformin").value; // Get Metformin value
    const Prednisone = document.getElementById("Prednisone").value; // Get Prednisone value
    const Syrup = document.getElementById("Syrup").value; // Get Syrup value

    const formData = new FormData(); // Create a new FormData object
    formData.append("maxRfidValue", maxRfidValue); // Append RFID value
    formData.append("Paracetamol", Paracetamol); // Append Paracetamol value
    formData.append("Aspirin", Aspirin); // Append Aspirin value
    formData.append("Metformin", Metformin); // Append Metformin value
    formData.append("Prednisone", Prednisone); // Append Prednisone value
    formData.append("Syrup", Syrup); // Append Syrup value

    fetch('insert_med.php', {
        method: 'POST',
        body: formData, // Send all form data including RFID data
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
            alert(data.message); // Show success message
            // You can also redirect or perform other actions here after successful insertion
        } else {
            alert(data.message); // Show error message if insertion fails
        }
    })
    .catch(error => {
        console.error('Error:', error);
